      ...
      for (unsigned long vtx_pos=0;vtx_pos<NUM_VTX;vtx_pos++) {
        ...
        for (auto& sel:br) {
          ...
          if (k==0) {
            ...
            position=sqrt(pow((*xyz_mom)[lar_pos][vtx_pos][0],2)+pow((*xyz_mom)[lar_pos][vtx_pos][1],2)
...
void FD_position_selection_histograms_changed()
{
  char eff[99];
  char caf[99];
  ...
  for (int vtx_pos=0;vtx_pos<NUM_VTX;vtx_pos++)
  {
    ...
    cs[vtx_pos]->Update();
    rs[vtx_pos]->Update();
    //cs[vtx_pos]->SaveAs(Form("/home/barwu/repos/MuonEffNN/10thTry/FD_hists_vtx=%f.png", vertex_position[vtx_pos]));
    //rs[vtx_pos]->SaveAs(Form("/home/barwu/repos/MuonEffNN/10thTry/FD_hists_ratios_vtx=%f.png", vertex_position[vtx_pos]));
  }
}
