...
prism_CAF_files="/storage/shared/wshi/CAFs/NDFHC/CAFv7_*"+argv[1]+".root"
allFiles=glob(prism_CAF_files)
...
def processFiles(f): #file loop
    # Analyse one file at a time, otherwise memory explodes!
    #f is only 1 file, each file get assigned to a different cpu
    #for f in f_list :
        output="/storage/shared/barwu/10thTry/combined/"+splitext(basename(f))[0]+"_Eff.root" #need to come up with a new place to put the TTrees
        if exists(output)==True:
            #print("testing")
            return None
        try:
            # Get caf TTree
            CAF=concatenate("{0}:cafTree".format(f), treeVarsToRead, library="np")
        ...
        for i_event in range(len(CAF['geoEffThrowResults'])):
            ...
            # Combined efficiency
            thisEff_combined=0.
            n_throws=len(CAF['geoEffThrowResults'][i_event][0][1])*64 #throws per 1 event
            #3 arrays to hold the reorientation of simulated neutrino collisions wihin the argon box
            #I feel like this can be integrated into the throw loop.
            random_y_throws=np.random.uniform(0,2*math.pi,n_throws)
            random_z_throws=np.random.uniform(-150.,150.,n_throws)
            random_phi_throws=np.random.uniform(0.,500.,n_throws)

        # Check which throws are in the FV. throws_FV is a boolean array with one element per throw.
            throws_FV=isFV_vec([CAF["vtx_x"][i_event]]*n_throws,random_y_throws,random_z_throws)
            ...
            for i_bitfield, bitfield in enumerate(CAF['geoEffThrowResults'][i_event][0][1]):
                ...
                throws_x=[CAF["vtx_x"][i_event]]*64
                # Get y for each random throw.
                throws_y=random_y_throws[i_bitfield*64:(i_bitfield+1)*64]
                # Get z for each random throw
                throws_z=random_z_throws[i_bitfield*64:(i_bitfield+1)*64]

                # Get rotation matrices from throws
                # Get phi for each random throw
                throws_phi=np.array(random_phi_throws[i_bitfield*64:(i_bitfield+1)*64])
                ...
                decayToTranslated=[[throws_x[i], throws_y[i], throws_z[i]] for i in range(len(throws_x))]
                ...
                phi_rot_vec=np.multiply(decayToTranslated, throws_phi[...,None])
...
# NOTE: IF saving the efficiencies to ROOT TTree for further processing, this is the place to Fill the TTree with the effs[i_event] variables.
# This may interfere with multi-processing, so I recommend doing this only with NUM_PROCS set to 1.
        with recreate(output) as tree:
            tree["event_data"]=newtree({'isCC':np.int32,
                                        'inFV':np.int32,
